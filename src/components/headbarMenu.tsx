...
const HeadbarMenu = () => {
    ...
    return (
        <header className="w-full h-14 flex items-center px-4">
            ...
            <div className={`fixed top-0 right-0 h-full w-[400px] bg-white transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'} z-40`}
            >
            ...
            {isOpen && (
                <div
                className="fixed inset-0 bg-black opacity-20 z-[-1]"
                onClick={() => setIsOpen(false)}
                />
            )}
        </header>
